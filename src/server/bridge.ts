...
export function bridge(component: BridgeComponentApi) {
	return function boundBridge<S extends string = string>(options?: BridgeOptions<S>) {
		...
		const deliverables = createDeliverableResource(component, options?.deliverables);
		const evaluations = createEvaluationResource(component, options?.evaluations);
